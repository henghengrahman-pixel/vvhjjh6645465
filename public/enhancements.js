...
function predictionCover({x,p}){
  const h=x.teams?.home||{},a=x.teams?.away||{};
  return `<a class="sarang-cover" href="/detail/${x.fixture.id}"><div class="cover-brand"><img src="${esc(state.boot?.settings?.logo||'/favicon.svg')}" alt="Rupiah Toto"></div><div class="cover-teams"><img src="${esc(h.logo||'/favicon.svg')}"><span>${realOrPredictedScore(p,x)}</span><img src="${esc(a.logo||'/favicon.svg')}"></div><strong>${esc(h.name)} vs ${esc(a.name)}</strong><time>${fmtDate(x.fixture.date)} • ${fmtTime(x.fixture.date)} WIB</time></a>`;
}
...
predictions=async function(){
  ...
  try{
    const today=wibDateValue(0),tomorrow=wibDateValue(1),allowedDates=new Set([today,tomorrow]),priority=[2,3,848,39,140,135,78,61,88,179],priorityOf=x=>{const i=priority.indexOf(Number(x.league?.id));return i<0?99:i},requests=await Promise.allSettled([loadFixtures('date='+today),loadFixtures('date='+tomorrow)]),rawByDate={};[today,tomorrow].forEach((date,i)=>{const rows=requests[i]?.status==='fulfilled'?requests[i].value:[];rawByDate[date]=[...new Map(rows.filter(x=>x.fixture?.id&&wibFixtureDate(x.fixture.date)===date&&x.teams?.home?.name&&x.teams?.away?.name).map(x=>[x.fixture.id,x])).values()].sort((a,b)=>priorityOf(a)-priorityOf(b)||new Date(a.fixture.date)-new Date(b.fixture.date)).slice(0,40)});const all=[...rawByDate[today],...rawByDate[tomorrow]],loaded=await Promise.all(all.map(async x=>{try{const d=await api('/api/football/predictions?fixture='+x.fixture.id);return{x,p:d.response?.[0]||null}}catch{return{x,p:null}}})),enriched=loaded.filter(z=>z.x.teams?.home?.name&&z.x.teams?.away?.name),fixtures=enriched.map(z=>z.x),dates=[today,tomorrow],first=today,last=tomorrow;
